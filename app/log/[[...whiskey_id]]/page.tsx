...
function LogPourPage() {
  ...
  useEffect(() => {
    ...
    createClient().from('whiskeys').select('*').eq('id', prefillId).single()
      .then(({ data }) => {
        if (data) { setSelectedWhiskey(data); setPriceTier((data.price_tier as PriceTier) ?? '') }
      })
  }, [user, prefillId])

  function handleWhiskeySelect(whiskey: Whiskey) {
